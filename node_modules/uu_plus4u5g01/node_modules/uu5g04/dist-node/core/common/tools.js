...
Tools.getChildrenFromUu5String = function (uu5String, opt) {
  ...
  while (matchS) {
    ...
    if (cIndex > pIndex) {
      ...
      pointer.content.push(pre ? _textEntityMap.TextEntityMap.replaceHtmlEntity(head) : Tools.replaceTextEntity(head));
    }
    var childTag = matchS[2];
    var tagObj = void 0;

    if (pre && !(childTag === 'uu5string.pre' && matchS[1] === '</')) {
      pointer.content.push(_textEntityMap.TextEntityMap.replaceHtmlEntity(matchS[0]));
    } else {
      ...
      if (matchS[1] === '</') {
        ...
        if (tagObj.tag !== childTag) {
          ...
          return _react2.default.createElement(UU5.Common.Error, // eslint-disable-line no-undef
        ...
        if (pre) {
          pre = false;
          pointer.content[pointer.content.length - 1] = _textEntityMap.TextEntityMap.replaceHtmlEntity(tagObj.content.join(''));
        } else {
          if (tagObj.forbidden) {
      ...
      //text after tag
      if (matchS[5]) pointer.content.push(pre ? _textEntityMap.TextEntityMap.replaceHtmlEntity(matchS[5]) : Tools.replaceTextEntity(matchS[5]));
    }
