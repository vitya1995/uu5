    ...
    switch (this.state.callFeedback) {
      ...
      case 'ready':
        mainChild = <Table {...this.getMainAttrs()}>
          <TBody>
          <Tr>
            {this.props.numbered && <Td className={this.getClassName().blockOfLineNumbers} mainAttrs={{ style: { width: this.state.blockOfLineNumbersWidth + 1 + "em" } }}>
                {this.state.blockOfLineNumbers}
              </Td>}
            <Td className={this.getClassName().blockOfLines}>{this.state.blockOfLines}</Td>
          </Tr>
          </TBody>
        </Table>;
        break;
