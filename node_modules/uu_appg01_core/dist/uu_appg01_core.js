...
/***/ function(module, exports, __webpack_require__) {
	...
	Client.prototype.invoke = function (method, uri, parameters, options) {
	  ...
	  function InvokeXhr(cmdClient, httpClientOpts, finalRequest, finalOpts) {
	    ...
	    return new Promise(function executor(resolve, reject) {
	      ...
	      if (finalOpts.transformResponse === false) xhr.responseType = "arraybuffer";
	      ...
	      xhr.onload = function (e) {
	        // status 200 - 5xx
	        var responseData = finalOpts.transformResponse === false ? new Uint8Array(xhr.response) : xhr.response;
	        if (logger.isDebugLoggable()) logger.debug("Received response.", { url: url, status: xhr.status, data: responseData });else logger.info("Received response.", { url: url, status: xhr.status });
	        resolve(interceptorMakeResponse(xhr, finalRequest, { data: responseData }));
	      };
	      ...
	      xhr.onerror = function (e) {
	        ...
	        reject(interceptorMakeResponse(xhr, finalRequest, { error: new _index.NetworkError("Network error - " + url), data: responseData }));
	      };
	      xhr.send(body);
	    });
	  }
	 ...
	 *    
	 * @example
	 *    // accept any response and don't parse it (response.data is Uint8Array containing bytes)
	 *    var workspace = "...";
	 *    var guestbookId = "...";
	 ...
	 *     unless option transformResponse is false (in which case it's Uint8Array containing bytes of the response body).
	 ...
	 * @param {boolean} options.transformResponse If false, processing of the response (such as parsing the response as JSON) will be skipped
	 *   and the response data will be array of bytes (Uint8Array). Default is true.
	 * @param {*} options.* Any other parameters are passed to the interceptors. These can include environmental options such
	 *   as "gatewayUri" which then take precedence over those from global environment.
	 * @return Promise resolving to a response object (or rejecting to a response object with error field).
...
/***/ function(module, exports, __webpack_require__) {
	...
	function UriBuilder() {
	  ...
	  ["scheme", "hostname", "vendor", "app", "subApp", "spp", "tid", "awid"].forEach(function (attr) {
	    ...
	    useCase: {
	      ...
	      set: function set(value) {
	        if (value != null) {
	          ...
	          if (value) {
	            UriParser.validateUseCase(value);
	          } else {
	            value = null;
	          }
	        }
